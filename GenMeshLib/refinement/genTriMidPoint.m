...
VTE = sparse(idx,jdx,v,Nvtx,Nvtx);
...
for p = 1:3
    ...
    r = mod(q,3) + 1;
    Vp = Elt(:,p);  
    Mpq = VTE((Elt(:,p)-1)*Nvtx + Elt(:,q));
    Mpr = VTE((Elt(:,p)-1)*Nvtx + Elt(:,r));
    NewElem(4*(ind - 1)+p,:) = [Vp,Mpq,Mpr];
end
M23 = VTE((Elt(:,2)-1)*Nvtx + Elt(:,3));
M31 = VTE((Elt(:,3)-1)*Nvtx + Elt(:,1));
M12 = VTE((Elt(:,1)-1)*Nvtx + Elt(:,2));
